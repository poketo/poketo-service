 ...
 * POST   /bookshelf/new
 * GET    /bookshelf/:slug
 * POST   (missing) /bookshelf/:slug/series/new
 * DELETE (missing) /bookshelf/:slug/series/:seriesId
...
app.use(
  route.get('/collection/:slug', async (ctx, slug) => {
    const collection = await Bookshelf.findOne({ slug });
...
app.use(
  route.get(
    ...
    async (ctx, siteId, seriesSlug) => {
      ...
      ctx.body = await poketo.getChapter(seriesUrl);
    },
  ),
);
