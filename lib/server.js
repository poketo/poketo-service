...
app.use(
  route.post('/collection/new', async ctx => {
    ...
    const newCollection = new Bookshelf({ series });
...
app.use(
  route.get('/collection/:id', async (ctx, collectionId) => {
    const collection = await Bookshelf.findById(ObjectId(collectionId));
    assert(collection, 404);
...
app.use(
  route.post('/collection/:collectionId/add', async (ctx, collectionId) => {
    ...
    assert(duplicateSeries, 204, `Series with url '${url}' already added to the collection`);
...
app.use(
  route.delete(
    '/collection/:collectionId/series/:seriesId',
    async (ctx, collectionId, seriesId) => {
      const collection = await Bookshelf.findById(ObjectId(collectionId));
      assert(collection, 404);
...
app.use(
  route.get(
    '/collection/:collectionId/series/:seriesId/:chapterSlug+',
    async (ctx, collectionId, seriesId, chapterId) => {
      ...
      assert(currentSeries, 404);

      ctx.body = await api.getSeriesChapter(series.url, seriesId, chapterId);;
    },
  ),
);
...
app.use(
  route.get(
    '/collection/:collectionId/markAsRead/:seriesId',
    async (ctx, collectionId, seriesId) => {
      const collection = await Bookshelf.findById(ObjectId(collectionId));
      assert(collection, 404);
      ...
      assert(currentSeriesIndex !== -1, 404);
