...
const MerakiScansAdapter: SiteAdapter = {
  ...
  parseUrl(url) {
    ...
    invariant(matches, `Could not parse url ${url}`);
    invariant(matches.seriesSlug, `Could not parse url ${url}`);
  ...
  async getSeries(seriesSlug) {
    ...
    // NOTE: Meraki returns a RSS feed for all series when the series slug can't
    // be found. The missing title block is a clue. If we can't parse the title,
    // chances are it was a 404.
    if (title === null) {
      throw new HTTPError(404, undefined, undefined, {});
    }
    ...
    const chapters: Array<ChapterPreview> = rssChapters.get().map(el => {
      ...
      const [,slug] = /\/([\d\.]+)\/\d+\/?$/.exec(chapterSlugText);
      const url = this.constructUrl(seriesSlug, slug);
      const id = utils.getChapterId(url);

      return { id, slug, number: slug, url, createdAt };
