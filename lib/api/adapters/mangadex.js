...
const normalizeJson = str => str.replace(/'/g, '"').replace(/,]$/, ']');
...
const extractChapters = (html: string, getChapterUrl: (slug: string) => string): ChapterMetadata[] => {
  ...
  const chapterDom = dom('tr[id]', '#chapters').get();
...
const MangadexAdapter: SiteAdapter = {
  ...
  async getSeries(seriesSlug) {
    ...
    const chapterPaginationText = dom('.table-responsive + p', '#chapters').first().text().trim();
