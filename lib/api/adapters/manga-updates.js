...
const MangaUpdatesAdapter: SiteAdapter = {
  ...
  parseUrl(url) {
    ...
    assert(/\/series.html\?id=/.test(url), 'Could not parse url');
    assert(matches.has('id'), 'Could not parse url');

    const seriesSlug = matches.get('id');
  ...
  constructUrl(seriesSlug, chapterSlug) {
    return `https://www.mangaupdates.com/series.html?id=${seriesSlug}`;
  },
