...
const MangaHereAdapter: SiteAdapter = {
  ...
  async getSeriesMetadata(seriesSlug) {
    const html = await utils.getPage(`http://www.mangahere.cc/manga/${seriesSlug}/`);
    const dom = cheerio.load(html);
    ...
    const chapters: Array<ChapterPreview> = dom('.title + ul > li', '.manga_detail > .detail_list').get().map(el => {
      ...
      const createdAt = getTimestamp(createdAtRawText);
  ...
  async getSeriesChapter(seriesSlug, chapterSlug) {
    const sourceUrl = `http://m.mangahere.cc/manga/${seriesSlug}/${chapterSlug}/`;
