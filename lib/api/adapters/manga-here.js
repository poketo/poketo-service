...
const throttledGetPage = throttle(getPage, 1, 500);
...
const MangaHereAdapter: SiteAdapter = {
  ...
  constructUrl(seriesSlug, chapterSlug) {
    return `http://www.mangahere.cc/manga/${seriesSlug}${chapterSlug ? `/${chapterSlug}/` : ''}`;
  },
  ...
  async getSeries(seriesSlug) {
    ...
    const seriesId = utils.hash(sourceUrl);
    ...
    const updatedAt = moment.tz(updatedAtRawText, 'MMM D, YYYY', TZ).unix();
    ...
    const chapters: Array<ChapterPreview> = dom('.title + ul > li', '.manga_detail > .detail_list').get().map(el => {
      ...
      const chapterSlug = href.match(/\/(c[\d|\.]+)\/?$/)[1];
      const chapterUrl = this.constructUrl(seriesSlug, chapterSlug);
      const chapterId = utils.hash(chapterUrl);
  ...
  async getChapter(seriesSlug, chapterSlug) {
    ...
    const body = await utils.getPage(sourceUrl.replace(/www\./, 'm.'));
    ...
    const seriesId = utils.hash(this.constructUrl(seriesSlug));
    const id = utils.hash(sourceUrl);
