...
const MangakakalotAdapter: SiteAdapter = {
  ...
  supportsUrl(url) {
    const isSupported =
      utils.compareDomain(url, 'http://mangakakalot.com') ||
      utils.compareDomain(url, 'http://manganelo.com');
    return isSupported;
  },
  ...
  parseUrl(url) {
    ...
    invariant(matches, 'Could not parse url');
    invariant(matches.seriesSlug, 'Could not parse url');
  ...
  async getSeries(seriesSlug) {
    ...
    const chapters: Array<ChapterPreview> = chapterRawData.map(
      (chapterData, i, arr) => {
        const prev = arr[i - 1];
